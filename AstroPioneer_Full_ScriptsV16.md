...
namespace AstroPioneer.Core
{
    ...
    public class Chunk
    {
        ...
        /// <summary>
        /// For machines with complex runtime state (Composter inventory, Battery charge,
        /// Water Pump orientation + flow rate, etc.)
        /// 
        /// RULE: byte[] buffers are pre-allocated at PLACEMENT TIME with fixed size
        /// (GameConstants.MACHINE_STATE_BUFFER_SIZE). Runtime mutations modify the
        /// existing buffer IN-PLACE. New byte[] is NEVER allocated per-frame.
        /// 
        /// Key = local grid position within this chunk.
        /// Value = pre-allocated byte buffer (modified in-place, serialized on Save).
        /// </summary>
        ...
        /// <summary>
        /// Allocate a state buffer for a newly-placed complex machine.
        /// Called ONCE at placement time, not per-frame.
        /// </summary>
        public byte[] AllocateComplexState(int localX, int localY)
        {
            ...
            if (ComplexStates.TryGetValue(key, out var existing))
                return existing; // Already allocated, reuse

            var buffer = new byte[GameConstants.MACHINE_STATE_BUFFER_SIZE];
        ...
        /// <summary>
        /// Get the pre-allocated state buffer for in-place read/write.
        /// Returns null if no complex state exists at this position.
        /// </summary>
        ...
        /// <summary>
        /// Remove complex state when a machine is destroyed.
        /// </summary>
        public void RemoveComplexState(int localX, int localY)
        {
            ComplexStates.Remove(new Vector2Int(localX, localY));
            IsDirty = true;
        }
    }
}
...
namespace AstroPioneer.Core
{
    ...
    /// 
    /// PERFORMANCE RULES:
    /// 1. ALL chunk data written via BinaryWriter (zero string allocation, zero GC).
    /// 2. ComplexStates (machine state) stored as raw byte[] â€” NO JSON AT ANY POINT.
    /// 3. File I/O happens on main thread but is fast because it's pure binary write.
    ///    If profiling shows issues, can be wrapped in Task.Run since BinaryWriter
    ///    is pure C# and does not touch Unity API.
    /// 
    /// File structure:
    ///   {persistentDataPath}/saves/
    ///   â”œâ”€â”€ meta.dat          â† world seed, player pos, day count, inventory
    ///   â”œâ”€â”€ entities.dat      â† all entity positions + state
    ///   â””â”€â”€ chunks/
    ///       â”œâ”€â”€ 0_0.chunk     â† binary chunk data
    ///       â”œâ”€â”€ 1_-2.chunk
    ///       â””â”€â”€ ...
    /// </summary>
    public static class SaveSystem
    {
        ...
        // â”€â”€â”€ Chunk Save Snapshot Job â”€â”€â”€

        private struct ChunkSnapshotJob : Unity.Jobs.IJob
        {
            public Unity.Collections.NativeArray<ushort> target;
            [Unity.Collections.ReadOnly] public Unity.Collections.NativeArray<ushort> source;
            public void Execute() => source.CopyTo(target);
        }

        /// <summary>
        /// Save a single chunk to disk using BinaryWriter.
        /// Called when chunk is unloaded OR on manual save.
        /// </summary>
        public static void SaveChunkBinary(Chunk chunk, int worldSeed)
        {
            ...
            // Snapshot Primitive Layers via Job System (Zero Main-Thread Stutter)
            ushort[] floorSnap = new ushort[totalCells];
            ushort[] utilSnap = new ushort[totalCells];
            ushort[] structSnap = new ushort[totalCells];
            byte[] metaSnap = new byte[totalCells];
            bool[] shadowSnap = new bool[totalCells];
            bool[] litSnap = new bool[totalCells];
            bool[] exploresSnap = new bool[totalCells];

            // Snapshot Metadata/Bool layers (Small enough for Main Thread)
            ...
            // Jobify heavy ushort[] copies (DOD best practice)
            var hFloor = new Unity.Collections.NativeArray<ushort>(chunk.FloorLayer.GetRawArray(), Unity.Collections.Allocator.TempJob);
            var hUtil = new Unity.Collections.NativeArray<ushort>(chunk.UtilityLayer.GetRawArray(), Unity.Collections.Allocator.TempJob);
            var hStruct = new Unity.Collections.NativeArray<ushort>(chunk.StructureLayer.GetRawArray(), Unity.Collections.Allocator.TempJob);
            
            var targetFloor = new Unity.Collections.NativeArray<ushort>(totalCells, Unity.Collections.Allocator.TempJob);
            var targetUtil = new Unity.Collections.NativeArray<ushort>(totalCells, Unity.Collections.Allocator.TempJob);
            var targetStruct = new Unity.Collections.NativeArray<ushort>(totalCells, Unity.Collections.Allocator.TempJob);

            var jobF = new ChunkSnapshotJob { source = hFloor, target = targetFloor }.Schedule();
            var jobU = new ChunkSnapshotJob { source = hUtil, target = targetUtil }.Schedule();
            var jobS = new ChunkSnapshotJob { source = hStruct, target = targetStruct }.Schedule();

            Unity.Jobs.JobHandle.CombineDependencies(jobF, jobU, jobS).Complete();

            targetFloor.CopyTo(floorSnap);
            targetUtil.CopyTo(utilSnap);
            targetStruct.CopyTo(structSnap);

            hFloor.Dispose(); hUtil.Dispose(); hStruct.Dispose();
            targetFloor.Dispose(); targetUtil.Dispose(); targetStruct.Dispose();

            // Extract non-jobbable ComplexStates
            ...
            // Async I/O with Error Handling (AAA Reliability)
            Task.Run(() =>
            {
                try
                {
                ...
                {
                    Debug.LogError($"[SaveSystem] Critical failure saving chunk {cx},{cy}: {e.Message}");
                }
            });
        }
...
namespace AstroPioneer.Machines
{
    ...
    public class MachineComposter : MonoBehaviour, IGridInteractable
    {
        ...
        private void PerformCatchUp(float elapsedSeconds)
        {
            ...
            while (totalAvailableTime >= processingTime && currentInputCount > 0 && currentOutputCount < maxOutputCapacity)
            {
                totalAvailableTime -= processingTime;
                currentInputCount--;
                currentOutputCount++;
            }

            currentProcessTimer = (currentInputCount > 0 && currentOutputCount < maxOutputCapacity) ? totalAvailableTime : 0f;
...
namespace AstroPioneer.Machines
{
    ...
    public class ProcessingStation : MonoBehaviour, IPowerConsumer
    {
        ...
        private void Update()
        {
            ...
            float delta = Time.deltaTime; // Assuming 1:1 real-to-game for now, or scale accordingly
            processProgress += delta / currentRecipe.craftTime;
        ...
        private void PerformCatchUp(float elapsedSeconds)
        {
            float totalProgressTime = (processProgress * currentRecipe.craftTime) + elapsedSeconds;
            
            if (totalProgressTime >= currentRecipe.craftTime)
            {
                CompleteCurrentRecipe();
            }
            else
            {
                processProgress = totalProgressTime / currentRecipe.craftTime;
            }
        ...
        private void DistributePower()
        {
            ...
            foreach (var consumer in consumers)
            {
                ...
                else if (consumerGraceTimers.TryGetValue(consumer, out float lastPoweredTime))
                {
                    ...
                    {
                        received = consumer.PowerRequired; // Maintain 'powered' state temporarily
                    }
                }
...
namespace AstroPioneer.Systems.Husbandry
{
    ...
    public class EnclosureSystem : MonoBehaviour
    {
            ...
            while (fillQueueBuffer.Count > 0)
            {
                ...
                for (int x = -1; x <= 1; x++)
                {
                    for (int y = -1; y <= 1; y++)
                    {
                        ...
                        if (Mathf.Abs(neighbor.x) < 500 && Mathf.Abs(neighbor.y) < 500) // Arbitrary map bounds for DOD
                        {
        ...
        {
            return Mathf.Abs(pos.x) >= 500 || Mathf.Abs(pos.y) >= 500;
        }
