    ...
    public class ChunkManager : MonoBehaviour
    {
        ...
        private void CompletePendingJobs()
        {
            ...
            foreach (var kvp in pendingChunks)
            {
                ...
                chunk.FloorLayer.CopyFrom(kvp.Value.arrays.floor.ToArray());
                chunk.UtilityLayer.CopyFrom(kvp.Value.arrays.utility.ToArray());
                chunk.StructureLayer.CopyFrom(kvp.Value.arrays.structure.ToArray());
                chunk.MetadataLayer.CopyFrom(kvp.Value.arrays.metadata.ToArray());

                // Bool layer needs manual copy (NativeArray<bool> → bool[])
                var shadowRaw = kvp.Value.arrays.shadow.ToArray();
                for (int i = 0; i < shadowRaw.Length; i++)
                {
                    if (shadowRaw[i])
...
namespace AstroPioneer.Core
{
    ...
    public class ChunkRenderer : MonoBehaviour
    {
        ...
        public void OnChunkLoaded(Chunk chunk)
        {
            ...
            var visuals = new List<SpawnedVisual>();
        ...
        public void UpdateVisualMetadata(int worldX, int worldY)
        {
            ...
            if (ServiceLocator.TryGet<ChunkManager>(out var cm) && cm.TryGetChunkAndLocal(worldX, worldY, out Chunk chunk, out int lx, out int ly))
            {
                ...
                if (visual != null && entry.category == StructureCategory.Crop && entry.sprites != null && entry.sprites.Length > 0)
                {
    ...
    public class EntityManager : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            for (int i = entities.Count - 1; i >= 0; i--)
            {
                ...
                if (entity == null) { entities.RemoveAt(i); continue; }
...
namespace AstroPioneer.Core
{
    ...
    {
        private static string SaveRoot => Path.Combine(Application.persistentDataPath, GameConstants.SAVE_DIR);
        private static string ChunkDir => Path.Combine(SaveRoot, GameConstants.SAVE_CHUNK_DIR);
        private static string MetaPath => Path.Combine(SaveRoot, GameConstants.SAVE_META_FILE);
        private static string EntitiesPath => Path.Combine(SaveRoot, GameConstants.SAVE_ENTITY_FILE);
        ...
        public static void SaveChunkBinary(Chunk chunk, int worldSeed)
        {
            ...
            using (var bw = new BinaryWriter(fs))
            {
                // Header
                bw.Write(worldSeed);
                bw.Write(chunk.Coord.x);
                bw.Write(chunk.Coord.y);
                bw.Write(chunk.IsGenerated);

                int totalCells = GameConstants.CHUNK_SIZE * GameConstants.CHUNK_SIZE;

                // Grid layers (flat array write — zero string allocation)
                WriteUshortArray(bw, chunk.FloorLayer.GetRawArray(), totalCells);
                WriteUshortArray(bw, chunk.UtilityLayer.GetRawArray(), totalCells);
                WriteUshortArray(bw, chunk.StructureLayer.GetRawArray(), totalCells);
                WriteByteArray(bw, chunk.MetadataLayer.GetRawArray(), totalCells);

                // Bool layers (pack 8 bools per byte for compression)
                WriteBoolArray(bw, chunk.ShadowLayer, totalCells);
                WriteBoolArray(bw, chunk.LitLayer, totalCells);
                WriteBoolArray(bw, chunk.ExploredLayer, totalCells);

                // Complex states (raw binary — NO JSON)
                bw.Write(chunk.ComplexStates.Count);
...
namespace AstroPioneer.Machines
{
    ...
    public class AgriMech : MonoBehaviour, IEntity
    {
        ...
        private void MoveMachine()
        {
            ...
            Vector3 nextWorldPos = transform.position + new Vector3(direction * 1f, 0, 0);
    ...
    public class CropManager : MonoBehaviour
    {
        ...
        public bool TryHarvestCropAt(Vector2Int gridPos, out CropData cropData)
        {
            ...
            if (stage >= 3)
            {
                ...
                {
                    cropData = saveMgr.allCropsRegistry.Find(c => c.structureID == id);
                }
...
namespace AstroPioneer.Managers
{
    ...
    public class GridManager : MonoBehaviour
    {
        ...
        public System.Collections.Generic.List<Vector2Int> GetNeighbors(Vector2Int pos)
        {
            var neighbors = new System.Collections.Generic.List<Vector2Int>(4);
            Vector2Int[] dirs = { Vector2Int.up, Vector2Int.down, Vector2Int.left, Vector2Int.right };
            foreach (var dir in dirs)
                neighbors.Add(pos + dir); // In infinite world, all positions are valid
            return neighbors;
        }
...
namespace AstroPioneer.Managers
{
    ...
    public class LocalizationManager : MonoBehaviour
    {
        ...
        public void LoadLanguage(string langCode)
        {
            ...
            // Load from Resources folder: Assets/Resources/Lang/en.json
            TextAsset textAsset = Resources.Load<TextAsset>($"Lang/{langCode}");
            
            if (textAsset != null)
            {
                // Simple TSV parser fallback (Key \t Value)
                ...
                Debug.Log($"[Localization] Loaded {localizedText.Count} strings for '{langCode}'");
            }
            else
            {
                Debug.LogWarning($"[Localization] Language file 'Lang/{langCode}' not found in Resources!");
            }
...
namespace AstroPioneer.Managers
{
    ...
    public class ObjectPoolManager : MonoBehaviour
    {
        ...
        // Pool storage
        private readonly Dictionary<string, Queue<GameObject>> poolQueues
            = new Dictionary<string, Queue<GameObject>>();

        // O(1) tracking to prevent double-return (replaces Queue.Contains)
        private readonly Dictionary<string, HashSet<GameObject>> poolTracking
        ...
        {
            EnsurePoolExists(poolTag);

            GameObject obj = null;

            // Try to dequeue a valid pooled object
            var queue = poolQueues[poolTag];
            var tracked = poolTracking[poolTag];
            ...
            while (queue.Count > 0)
            {
                ...
                // Object may have been externally destroyed
                if (obj != null) break;
                obj = null;
            }

            // Pool was empty or all objects were destroyed
            if (obj == null)
        ...
        /// <summary>
        /// Return an object to the pool. O(1) duplicate check via HashSet.
        /// </summary>
        public void ReturnToPool(string poolTag, GameObject obj)
        {
            ...
            // O(1) duplicate check — prevents double-return corruption
            if (tracked.Contains(obj)) return;

            obj.SetActive(false);
            poolQueues[poolTag].Enqueue(obj);
            tracked.Add(obj);
        }

        /// <summary>
        /// Pre-allocate objects into the pool at scene start.
        /// Prevents initial frame lag from first-time Instantiate calls.
        /// </summary>
        ...
        {
            return poolQueues.TryGetValue(poolTag, out var queue) ? queue.Count : 0;
        }

        // ─── Internal ───

        private void EnsurePoolExists(string poolTag)
        {
            if (!poolQueues.ContainsKey(poolTag))
            {
                poolQueues[poolTag] = new Queue<GameObject>();
                poolTracking[poolTag] = new HashSet<GameObject>();
            }
        }
    }
}
...
namespace AstroPioneer.Player
{
    ...
    public class PlayerToolState : MonoBehaviour
    {
        ...
        void HandleGridClick(Vector2Int gridPos)
        {
            ...
            if (selectedItem != null && selectedItem.type == ItemType.Tool && selectedItem.id.ToLower().Contains(Core.GameConstants.TOOL_HOE))
            {
            ...
            switch (selectedItem.type)
            {
                ...
                case ItemType.Tool:
                    if (!selectedItem.id.ToLower().Contains(Core.GameConstants.TOOL_HOE))
                        WaterCrop(gridPos);
                    break;
    ...
    public class RescueProtocol : MonoBehaviour
    {
        ...
        private IEnumerator RescueSequence()
        {
            ...
            if (playerMove != null && botE != null)
                botE.ExecuteEmergencyRescue(playerMove.transform.position, playerMove.transform.position);
