...
{
    /// <summary>
    /// ChunkRenderer — Bridges DATA (ushort IDs in Chunk) to VISUAL (GameObjects in scene).
    /// 
    /// Uses Object Pool exclusively:
    /// - OnChunkLoaded: iterate cells → spawn visuals from pool based on StructureEntry
    ...
    public class ChunkRenderer : MonoBehaviour
    {
        ...
        // Key: ChunkCoord, Value: list of (localPos, poolTag, gameObject)
        ...
        public void OnChunkUnloaded(ChunkCoord coord)
        {
            ...
            if (ServiceLocator.TryGet<ObjectPoolManager>(out var pool))
            {
                ...
                {
                    if (v.go != null)
                        pool.ReturnToPool(v.poolTag, v.go);
                }
            }
        ...
        public void SpawnVisualAt(int worldX, int worldY, ushort structureID)
        {
            ...
            string poolTag = $"Structure_{structureID}";

            GameObject visual = null;
            if (ServiceLocator.TryGet<ObjectPoolManager>(out var pool))
            {
                visual = pool.SpawnFromPool(poolTag, entry.visualPrefab, worldPos);
            }
        ...
        public void DespawnVisualAt(int worldX, int worldY)
        {
            ...
            for (int i = visuals.Count - 1; i >= 0; i--)
            {
                ...
                {
                    if (ServiceLocator.TryGet<ObjectPoolManager>(out var pool))
                        pool.ReturnToPool(visuals[i].poolTag, visuals[i].go);
        ...
        private void SpawnLayerVisual(Chunk chunk, GridLayer<ushort> layer, int x, int y, List<SpawnedVisual> visuals)
        {
            ...
            {
                localX = x, localY = y,
                poolTag = poolTag, go = visual, spriteRenderer = visual.GetComponent<SpriteRenderer>()
            });
        }
...
namespace AstroPioneer.Core
{
    ...
    public class EntityManager : MonoBehaviour
    {
        ...
        private readonly Dictionary<AstroPioneer.Interfaces.IEntity, ChunkCoord> entityChunks
            = new Dictionary<AstroPioneer.Interfaces.IEntity, ChunkCoord>();
        ...
        public void Register(AstroPioneer.Interfaces.IEntity entity)
        {
            if (!entities.Contains(entity))
            {
                entities.Add(entity);
                // Track initial chunk
                var chunk = ChunkCoord.FromWorldPos(
                    entity.WorldPosition.x, entity.WorldPosition.y);
        ...
        // ─── Per-Frame Chunk Boundary Check ───
        ...
        void LateUpdate()
        {
            for (int i = entities.Count - 1; i >= 0; i--)
            {
                ...
                var currentChunk = ChunkCoord.FromWorldPos(
                    entity.WorldPosition.x, entity.WorldPosition.y);

                if (entityChunks.TryGetValue(entity, out var lastChunk) && lastChunk != currentChunk)
                {
                    entity.OnChunkExited(lastChunk);
                    entity.OnChunkEntered(currentChunk);
                    entityChunks[entity] = currentChunk;
                }
            }
        }
        ...
        public static Vector2Int GetGridBelowEntity(Vector3 entityWorldPos)
        {
            return new Vector2Int(
                Mathf.FloorToInt(entityWorldPos.x),
                Mathf.FloorToInt(entityWorldPos.y));
        }
...
namespace AstroPioneer.Core
{
    ...
    /// 2. ComplexStates (machine state) stored as raw byte[] — NO JSON AT ANY POINT.
    ...
    ///   ├── meta.dat          ← world seed, player pos, day count, inventory
    ///   ├── entities.dat      ← all entity positions + state
    ///   └── chunks/
    ///       ├── 0_0.chunk     ← binary chunk data
    ///       ├── 1_-2.chunk
    ///       └── ...
    /// </summary>
    public static class SaveSystem
    {
        ...
        public static void SaveChunkBinary(Chunk chunk, int worldSeed)
        {
            ...
            // Deep Copy Snapshot on Main Thread
            ...
            Task.Run(() =>
            {
                ...
                using (var bw = new BinaryWriter(fs))
                {
                    ...
                    foreach (var kvp in complexStatesSnap)
                    {
                        ...
                        bw.Write(kvp.Value); // Write raw byte[] directly
                    }
                }
            });
        }
...
namespace AstroPioneer.Data
{
    ...
    /// <summary>
    /// StructureEntry — Data-driven definition mapping a ushort ID to its visual and config.
    /// 
...
{
    /// <summary>
    /// StructureRegistry — Singleton ScriptableObject that maps ushort IDs
...
namespace AstroPioneer.Machines
{
    ...
    public class ProcessingStation : MonoBehaviour, IPowerConsumer
    {
        ...
        private IEnumerator ProcessItem()
        {
            ...
            {
                while (!InventoryManager.Instance.AddItem(currentRecipe.resultItem, currentRecipe.resultQuantity))
                    yield return null;
            }
...
namespace AstroPioneer.Machines.Automation
{
    ...
    /// 
    /// Classified as ENTITY — moves freely in float-space.
    /// </summary>
...
namespace AstroPioneer.Managers
{
    ...
    /// 3. Uses ServiceLocator instead of static Instance singleton
    /// </summary>
